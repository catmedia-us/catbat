'--------------------------------------------------------------------------------
' $Id: VBXCopy.vbs,v 1.6 2010/01/31 20:33:20 keilw Exp $
'--------------------------------------------------------------------------------
...
' Version 1.0.12
'
' Copyright 2000-2016 Creative Arts & Technologies. Alle Rechte vorbehalten.
'
...
'On Error Resume Next
	...
	If stDrive <> "\" Then
		If Not FSO.DriveExists(stDrive) Then 
			s = "Ausgangs-Laufwerk (" & stDrive & ") ist derzeit nicht bereit."
			'WScript.Echo s
	...
	If stTargetDrive <> "\" Then
		If Not FSO.DriveExists(stTargetDrive) Then			
			s = "Ziel-Laufwerk (" & stTargetDrive & ") ist derzeit nicht bereit."
			'WScript.Echo s
			WshShell.Popup s, , AppTitle, PopUpButtonOK + PopUpIconStop
			Exit Function
		End If
...
Function BuildTestDirectory(FSO)
	...
	Set TextStream = FSO.CreateTextFile(stFilePath & "\Liesmich.txt")
...
' Forcing an explicit REPLACE of existing Folders (for Backup on certain media like DVD-RAM)
	...
	If iShow = ShowResultInfo Then
			...
			Else
				s = s & NewLine & NewLine & "(Ende nach " & DelayDuration & " Sekunden)"
	...
	Else
		If iShow = ShowResultAll Or iShow = ShowResultStart Then 
			'WScript.Echo s
			s = s & NewLine & NewLine & "(Fortsetzung nach " & DelayDuration & " Sekunden)"
			'if bKeep Then
			'	WshShell.Popup s, , AppTitle, PopUpButtonOK + PopUpIconInfo
			'Else
	...
	If iShow = ShowResultAll Or iShow = ShowResultFinal Then 
		If Len(s)>0 Then 					
			...
			If (bSkip And stSkippedItem <> "") Then
				s = s & NewLine & "Übersprungen: " & stSkippedItem
			End If
			'WScript.Echo s
			If bKeep Then
				WshShell.Popup s, , AppTitle, PopUpButtonOK + PopUpIconInfo
